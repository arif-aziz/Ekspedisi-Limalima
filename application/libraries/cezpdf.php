...
function Cezpdf($paper='a4',$orientation='landscape'){
	...
	$this->ez['topMargin']=140;
	$this->ez['bottomMargin']=75;
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  $defaults = array(
    'shaded'=>1,'showLines'=>1,'shadeCol'=>array(0.8,0.8,0.8),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>10,'titleFontSize'=>18
    ,'titleGap'=>15,'lineCol'=>array(0,0,0),'gap'=>5,'xPos'=>'centre','xOrientation'=>'centre'
...
//        $x1=$x1+$dm;
        ...
        $y = $this->ez['pageHeight']-$this->ez['topMargin']; //menentukan ukuran tabel (top)
        $y0=$y+$decender;
        $mx=0;
        if ($options['showHeadings']){
...
function ezImage($image,$pad = 5,$width = 0,$resize = 'full',$just = 'center',$border = ''){
	...
	if (stristr($image,'://'))//copy to temp file
	{
		...
		while(!feof($fp))
   		{
				$cont.= fread($fp,1024);
      			// $cont = fread($fp,1024); //edit by farhan
   		}
   		fclose($fp);
