...
function setCurrentFont(){
  if (strlen($this->currentBaseFont)==0){
    ...
    $this->selectFont('./include/fonts/Helvetica.afm');
  }
...
function stream($options=''){
  ...
  $fileName = (isset($options['Content-Disposition'])?$options['Content-Disposition'].".pdf":'file.pdf');
...
function getFontHeight($size){
  if (!$this->numFonts){
    $this->selectFont('./include/fonts/Helvetica');
  }
...
function addText($x,$y,$size,$text,$angle=0,$wordSpaceAdjust=0){
  if (!$this->numFonts){$this->selectFont('./include/fonts/Helvetica');}
